...
#+lispworks
(defvar *fifo-queue-table* (make-hash-table :test 'equalp :single-thread nil))
...
(defstruct (fifo-queue
  ...
  (valet-lot nil)
  (parking-spots nil)
  (valet-queue #+sbcl (sb-queue:make-queue)
               #+lispworks (make-empty-queue))
  (agent-table #+sbcl (make-hash-table :synchronized t :test 'equalp)
               #+lispworks (make-hash-table :test 'equalp))
  (agent-queue (make-empty-queue))
  (caller-table #+sbcl (make-hash-table :synchronized t :test 'equalp)
                #+lispworks (make-hash-table :test 'equalp))
...
(defun show-agents ()
  (with-locked-hash-table (*fifo-queue-table*)
    (maphash #'(lambda (k v)
			  ...
			  (progn
			    (when (caller-parking-spot caller)
			      #+sbcl
                              ...
                              #+lispworks
                              (enqueue-1 (valet-queue q) (caller-parking-spot caller))
...
(defmethod send-caller-to-last-resort ((q fifo-queue) caller)
  ...
  (when (caller-parking-spot caller)
    #+sbcl (sb-queue:enqueue (caller-parking-spot caller) (valet-queue q))
    #+lispworks (enqueue-1 (valet-queue q) (caller-parking-spot caller))
    (setf (caller-parking-spot caller) nil))
...
(defmethod place-caller-in-queue ((q fifo-queue) input status)
  (with-recursive-lock-held ((lock *session*))
    ...
    (let ((caller (create-fifo-caller q :session *session*)))
      (handler-case
	  (let ((parking-spot #+sbcl (sb-queue:dequeue (valet-queue q))
                              #+lispworks (dequeue (valet-queue q))))
	    (if parking-spot
		(progn
  ...
  (cond ((gethash name *fifo-queue-table*)
	...
	(t
	 (let ((q (make-fifo-queue :queue-name name
	   ...
	   (loop for x from valet-lot-start to valet-lot-end
              do
                #+sbcl (sb-queue:enqueue x (valet-queue q))
                #+lispworks (enqueue-1 (valet-queue q) x))
